...
use reqwest::{
    header::{HeaderMap, HeaderValue, COOKIE, CONTENT_TYPE}, 
...
use crate::{
    UnifiError, UnifiResult, LoginRequest, ApiResponse, EmptyResponse,
    VoucherApi, SiteApi,
};
...
impl ClientConfigBuilder {
    ...
    pub fn build(self) -> UnifiResult<ClientConfig> {
        ...
        let url = Url::parse(&controller_url)
            .map_err(|e| UnifiError::ConfigurationError(format!("Invalid controller URL: {}", e)))?;
...
impl UnifiClient {
    ...
    pub async fn login(&mut self, password: Option<String>) -> UnifiResult<()> {
        let password = match (password, &self.config.password) {
            ...
            (None, None) => prompt_password("Enter UniFi controller password: ")
                .map_err(|e| UnifiError::AuthenticationError(format!("Failed to read password: {}", e)))?,
        };
        
        let login_url = self.config.controller_url.join("/api/login")
        ...
        if !response.status().is_success() {
            return Err(UnifiError::AuthenticationError(
                format!("Authentication failed with status code: {}", response.status())
            ));
        }
        ...
        let cookie_header = response.headers()
            .get("set-cookie")
            .ok_or_else(|| UnifiError::AuthenticationError("No cookies received from server".into()))?
            .to_str()
    ...
    pub(crate) async fn request<T, R>(&mut self, method: Method, endpoint: &str, body: Option<T>) -> UnifiResult<R>
    where
        T: Serialize,
        R: DeserializeOwned,
    {
        ...
        headers.insert(COOKIE, HeaderValue::from_str(&auth_state.cookies)
        ...
        if !response.status().is_success() {
            return Err(UnifiError::ApiError(
                format!("API request failed with status code: {}", response.status())
            ));
        }
